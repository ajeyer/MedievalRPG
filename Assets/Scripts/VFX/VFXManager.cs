...
public class VFXManager : SingletonMonobehaviour<VFXManager>
{
    ...
    private void displayHarvestActionEffect(Vector3 effectPosition, HarvestActionEffect harvestActionEffect)
    {
        switch (harvestActionEffect)
        {
            ...
            case HarvestActionEffect.leavesFalling:
                GameObject leavesFalling = PoolManager.Instance.ReuseObject(leavesFallingPrefab, effectPosition, Quaternion.identity);
                leavesFalling.SetActive(true);
                StartCoroutine(DisableHarvestActionEffect(leavesFalling, twoSeconds));
                break;
